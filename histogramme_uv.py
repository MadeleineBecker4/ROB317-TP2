...
while(ret):
    ...
    #cv2.imshow('Histogram visible by human', HumanVisibleHistogrammeUV / (HumanVisibleHistogrammeUV.max()-HumanVisibleHistogrammeUV.min()))
    ...
    if index > nbFrames:
        ...
        i = index-nbFrames//2
        dist_Correl[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_CORREL)
        # dist_ChiSquare[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_CHISQR)
        # dist_Intersection[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_INTERSECT)
        # dist_Bhattacharyya[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_BHATTACHARYYA)
        # dist_Hellinger[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_HELLINGER)
        # dist_ChiSquareAlt[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_CHISQR_ALT)
        # dist_KLDiv[i] = cv2.compareHist(histPrev, histNext, cv2.HISTCMP_KL_DIV)
